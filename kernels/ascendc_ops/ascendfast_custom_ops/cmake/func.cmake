...
function(add_simple_kernel_compile)
  ...
  if (${ASCEND_PACK_SHARED_LIBRARY})
    if (NOT TARGET op_kernel_pack)
      ...
      add_dependencies(op_kernel_pack ascendc_bin_${BINCMP_COMPUTE_UNIT}_gen_ops_config)
    endif()
  endif()
...
function(add_opregistry_target)
  ...
  add_custom_target(op_registry_pack
                    ...
                    --registry-file-path=${ASCEND_AUTOGEN_PATH}
...
function(add_static_library STATIC_LIB_NAME)
  ...
  file(REMOVE_RECURSE ${FILE_LIST_TMP_DIR})
  file(MAKE_DIRECTORY ${FILE_LIST_TMP_DIR})

  set(FILE_LIST "")
  set(TARGET_LIST "")
  foreach(TARGET IN LISTS ARGN)
    set(CUR_FILE ${FILE_LIST_TMP_DIR}/${TARGET}.txt)
    list(APPEND FILE_LIST ${CUR_FILE})
  ...
  string(JOIN " " FILE_LIST_PARAM ${FILE_LIST})
  ...
  add_custom_command(
    OUTPUT ${OUTPUT_LIB}
    COMMAND ${ASCEND_PYTHON_EXECUTABLE} ${CMAKE_SOURCE_DIR}/cmake/util/ascendc_gen_static_library.py
    --object-files=${FILE_LIST_PARAM}
    --kernel-libs=${KERNEL_LIBS}
    --tmp-obj-dir=${COLLECT_TMP_DIR}
    --output-file=${OUTPUT_LIB}
    --remove-tmp-files=1
    DEPENDS ${TARGET_LIST} ${FILE_LIST}
    COMMENT "Merging static libraries into ${OUTPUT_LIB}"
    VERBATIM
  )
  add_custom_target(${STATIC_LIB_NAME} ALL DEPENDS ${OUTPUT_LIB})
endfunction()

function(add_vendor_cmake VENDOR_NAME OUTPUT_PATH STATIC_LIB_NAME DYNAMIC_LIB_NAME)
  file(MAKE_DIRECTORY ${OUTPUT_PATH})
  execute_process(
    COMMAND ${ASCEND_PYTHON_EXECUTABLE} ${CMAKE_SOURCE_DIR}/cmake/util/ascendc_gen_vendor_cmake.py
    --vendor-name=${VENDOR_NAME}
    --output-path=${OUTPUT_PATH}
    --static-lib-name=${STATIC_LIB_NAME}
    --dynamic-lib-name=${DYNAMIC_LIB_NAME}
    RESULT_VARIABLE result
  )

  if(NOT result EQUAL 0)
    message(FATAL_ERROR "Create config.cmake and targets.cmake failed.")
  endif()
endfunction()
